...
class MatNpu : public cv::Mat {
    ...
    private: 
        ...
        MatNpu(int32_t rows, int32_t cols, int32_t type, T* data, 
    ...
    public: 
        ...
        MatNpu<To> matmul(MatNpu<Ti> mat, int32_t output_type, bool AC_native, bool B_native) {
            _rknn_matmul_type mm_type = choose_matmul_type(this->type(), mat.type(), output_type);
            tensor_result result = matmul_npu<To, T, Ti>(rows, cols, mat.cols, mm_type, data, mat.data, AC_native, B_native);
            return MatNpu(
                rows, mat.cols, output_type, (To*) result.resultMatrix->virt_addr,
